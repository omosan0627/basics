...
Lemma shift_zero: forall t c, shift c 0 t = t.
induction t.
- intros. simpl. auto.
- intros. simpl. rewrite IHt. auto.
- intros. simpl. rewrite IHt. auto.
- intros. simpl. rewrite IHt1. rewrite IHt2. auto.
- intros. simpl. destruct (c <=? n). assert (n + 0 = n). lia. rewrite H. auto. auto.
- intros. simpl. rewrite IHt. auto.
- intros. simpl. rewrite IHt. auto.
Qed.
...
Lemma count_env_var_bind_reverse: forall m Γ, count_env_var_bind Γ =
count_env_var_bind (env_reverse_shift m Γ). intros ? ?. generalize dependent m.
induction Γ. intros. simpl. auto. intros. destruct a. simpl. rewrite IHΓ with (m:=m). auto.
simpl. apply IHΓ. Qed.
...
Lemma has_free_var_in_type_shift: forall t T c, type_shift c 1 t = T
-> has_free_var_in_type c T = false.
induction t.
- intros. simpl in H. subst. simpl. auto.
- intros. simpl in H. remember (c <=? n) as b. symmetry in Heqb. destruct b.
  subst. simpl. apply Nat.leb_le in Heqb. assert ((c =? n + 1) = false).
  apply Nat.eqb_neq. lia. rewrite H. auto. subst. simpl.
  apply Nat.leb_gt in Heqb. assert ((c =? n) = false). apply Nat.eqb_neq. lia.
  rewrite H. auto.
- intros. simpl in H. inversion H. simpl. rewrite IHt1. rewrite IHt2. simpl. auto.
  auto. auto.
- intros. simpl in H. subst. simpl. apply IHt. auto.
Qed.

Lemma has_free_var_in_type_get_var_type_shift: forall Γ k T, 
get_var_type k (env_shift 0 1 Γ) = Some T -> has_free_var_in_type 0 T = false.
induction Γ.
- intros. simpl in H. inversion H.
- intros. destruct k, a. simpl in H. inversion H. apply has_free_var_in_type_shift in H1.
  inversion H. subst. auto.
  simpl in H. apply IHΓ with (k:=0). auto. 
  simpl in H. apply IHΓ with (k:=k). auto.
  simpl in H. apply IHΓ with (k:=S k). auto.
Qed.
...
- intros. destruct Γ'. simpl in HeqΓ''. inversion HeqΓ''. simpl in H1. rewrite H1.
  ...
  auto. apply has_free_var_in_type_get_var_type_shift in H. auto. rewrite H3 in H.
  ...
  rewrite H13 in H11. auto. apply has_free_var_in_type_get_var_type_shift in H. auto. rewrite H2. 
...
Lemma env_one_more_var_bind: forall Γ Γ' T, env (Γ ++ Γ') -> env (Γ ++ env_var_bind T :: Γ').
intros. remember (Γ ++ Γ') as Γ''. generalize dependent Γ. generalize dependent Γ'. generalize dependent T.
induction H.
- intros. destruct Γ. simpl in HeqΓ''. simpl. subst. constructor. constructor.
simpl in HeqΓ''. inversion HeqΓ''.
- intros. destruct Γ0. simpl in HeqΓ''. destruct Γ'. inversion HeqΓ''. inversion HeqΓ''.
  simpl. constructor. constructor. auto. simpl in HeqΓ''. inversion HeqΓ''.
  simpl. assert (Γ = env_reverse_shift 0 (Γ0 ++ Γ')). symmetry in H2. rewrite H2.
  rewrite env_shift_inversion. auto. rewrite env_reverse_shift_split in H0.
  pose proof (IHenv (type_reverse_shift 0 T) _ _ H0). apply env_type_bind_cons in H3.
  rewrite env_shift_split in H3. rewrite env_shift_inversion_reverse in H3.
  simpl in H3. rewrite env_shift_inversion_reverse in H3.
  rewrite type_shift_inversion_reverse in H3. admit.
Admitted.
...
Lemma get_var_type_var_bind_larger: forall Γ' Γ0 n k T T0, get_var_type k (Γ' ++ Γ0) = Some T
-> n = count_env_var_bind Γ' -> n <= k -> get_var_type (k+1) (Γ' ++ env_var_bind T0 :: Γ0) = Some T.
induction Γ'.
- intros. simpl. simpl in H. assert ((k+1=?0)=false). apply Nat.eqb_neq. lia.
  rewrite H2. assert (Init.Nat.pred (k+1)=k). lia. rewrite H3. auto.
- intros. destruct a. simpl. assert ((k+1=?0)=false). apply Nat.eqb_neq. lia.
  rewrite H2. assert (Init.Nat.pred (k+1)=(Init.Nat.pred k + 1)). simpl in H0. lia.
  rewrite H3. apply IHΓ' with (n-1). simpl in H0. simpl in H.
  assert ((k=?0)=false). apply Nat.eqb_neq. lia. rewrite H4 in H. auto. simpl in H0. lia.
  lia. simpl. rewrite IHΓ' with (n:=n)(T:=T). auto. simpl in H. auto. auto. auto.
Qed.

Lemma get_var_type_var_bind_smaller: forall Γ' Γ0 n k T T0, get_var_type k (Γ' ++ Γ0) = Some T
-> n = count_env_var_bind Γ' -> n > k -> get_var_type k (Γ' ++ env_var_bind T0 :: Γ0) = Some T.
induction Γ'.
- intros. simpl in H0. lia.
- intros. destruct a. simpl. simpl in H. remember (k=?0) as b. symmetry in Heqb. destruct b.
  auto. apply IHΓ' with (n:=(n-1)). auto. simpl in H0. lia. apply Nat.eqb_neq in Heqb.
  lia. simpl. simpl in H. apply IHΓ' with (n:=n)(T:=T). auto. simpl in H0. auto. auto.
Qed.

(* おかしい。Tがtype_shift 0 m Tであるべき*)
Lemma env_type_shift_and_shift_prep1: forall Γ Γ' s S T n, Γ' ++ Γ |- s : S ->
n = count_env_var_bind Γ' ->
Γ' ++ env_var_bind T :: Γ |- shift n 1 s : S.
intros. remember (Γ' ++ Γ) as Γ''. generalize dependent T. generalize dependent Γ'.
generalize dependent Γ. generalize dependent n. induction H.
- intros. simpl. constructor. apply env_one_more_var_bind. symmetry in HeqΓ''. rewrite HeqΓ''.
  auto.
- intros. simpl. constructor. apply env_one_more_var_bind. symmetry in HeqΓ''. rewrite HeqΓ''.
  auto. apply IHtype_bind. auto. auto.
- intros. simpl. remember (n <=? k) as b. destruct b. symmetry in Heqb. constructor.
  apply env_one_more_var_bind. symmetry in HeqΓ''. rewrite HeqΓ''. auto.
  apply get_var_type_var_bind_larger with (n:=n). rewrite HeqΓ'' in H0. auto. auto.
  apply Nat.leb_le in Heqb. auto. constructor. apply env_one_more_var_bind. symmetry in HeqΓ''.
  rewrite HeqΓ''. auto. apply get_var_type_var_bind_smaller with (n:=n). rewrite HeqΓ'' in H0.
  auto. auto. symmetry in Heqb. apply Nat.leb_gt. auto. 
- simpl. constructor. apply env_one_more_var_bind. symmetry in HeqΓ''. rewrite HeqΓ''. auto.
 apply (IHtype_bind (S n) Γ0 (env_var_bind T1 :: Γ')).
  rewrite HeqΓ''. simpl. auto. simpl. lia.
- simpl. intros. apply TApp' with (T11:=T11). apply env_one_more_var_bind. symmetry in HeqΓ''. 
  rewrite HeqΓ''. auto. apply IHtype_bind1.
  auto. auto. apply IHtype_bind2. auto. auto.
- simpl. intros. constructor. apply env_one_more_var_bind. symmetry in HeqΓ''. rewrite HeqΓ''. auto.
  pose proof (IHtype_bind n (env_shift 0 1 Γ0) (env_type_bind :: env_shift 0 1 Γ')).
  simpl in H2. rewrite env_shift_split. simpl. apply H2. rewrite HeqΓ''.
  rewrite env_shift_split. auto. pose proof (count_env_var_bind_shift). symmetry in H3.
  rewrite H3. auto.
- intros. simpl. constructor. apply env_one_more_var_bind. rewrite HeqΓ'' in H. auto.
...
Lemma get_var_type_env_type_bind: forall k Γ' Γ0 T, get_var_type k (Γ' ++ Γ0) = Some T ->
get_var_type k (Γ' ++ env_type_bind :: Γ0) = Some T.
intros ? ?. generalize dependent k. induction Γ'.
intros. simpl. simpl in H. auto.
intros. induction a. simpl. simpl in H. remember (k=?0) as b. destruct b. auto.
apply IHΓ'. auto. simpl. simpl in H. apply IHΓ'. auto.
Qed.

Lemma type_shift_swap_mplus: forall T m c, type_shift (m+c+1) 1 (type_shift c 1 T)
= type_shift c 1 (type_shift (m+c) 1 T).
induction T.
- intros. simpl. auto.
- intros. simpl. remember (c <=? n) as b1. remember (m + c <=? n) as b2.
  symmetry in Heqb1. symmetry in Heqb2. destruct b1, b2.
  simpl. apply Nat.leb_le in Heqb1, Heqb2. assert ((m + c + 1 <=? n + 1) = true).
  apply Nat.leb_le. lia. assert ((c <=? n + 1) = true). apply Nat.leb_le. lia. rewrite H.
  rewrite H0. auto. simpl. rewrite Heqb1. apply Nat.leb_gt in Heqb2.
  assert ((m + c + 1 <=? n + 1) = false). apply Nat.leb_gt. lia. rewrite H. auto.
  apply Nat.leb_gt in Heqb1. apply Nat.leb_le in Heqb2. lia.
  simpl. rewrite Heqb1. assert ((m+c+1 <=?n) = false). apply Nat.leb_gt.
  apply Nat.leb_gt in Heqb2. lia. rewrite H. auto.
- intros. simpl. rewrite IHT1. rewrite IHT2. auto.
- intros. simpl. pose proof (IHT m (c+1)). assert (m + (c+1) = m+c+1). lia. rewrite H0 in H.
  rewrite H. auto.
Qed.

Lemma env_shift_swap_mplus: forall Γ m c, env_shift (m+c+1) 1 (env_shift c 1 Γ)
= env_shift c 1 (env_shift (m+c) 1 Γ).
induction Γ.
- intros. simpl. auto.
- intros. destruct a. simpl. rewrite IHΓ. rewrite type_shift_swap_mplus. auto.
  simpl. rewrite IHΓ. auto.
Qed.
...
- intros. simpl. constructor. apply env_one_more_type_bind. rewrite HeqΓ'' in H. auto. auto.
 apply IHtype_bind. auto. auto.
- intros. simpl. constructor. apply env_one_more_type_bind. rewrite HeqΓ'' in H. auto. auto.
  apply get_var_type_and_shift. apply get_var_type_env_type_bind. rewrite HeqΓ'' in H0. auto.
- intros. simpl. constructor. apply env_one_more_type_bind. rewrite HeqΓ'' in H. auto. auto.
...
- intros. simpl. constructor. apply env_one_more_type_bind. rewrite HeqΓ'' in H. auto. auto. 
  ...
  simpl in H2. assert (m=m+0). lia. rewrite H3. pose proof env_shift_swap_mplus.
  symmetry in H4. rewrite H4. symmetry in H3. rewrite H3. rewrite env_shift_split.
  simpl. apply H2. rewrite HeqΓ''. rewrite env_shift_split. auto. pose proof count_env_type_bind_shift.
  symmetry in H5. rewrite H5. lia.
- intros. simpl. pose proof TTApp. simpl in IHtype_bind.
  ...
  assert (m = m+0). lia. rewrite H4. apply type_shift_swap_mplus.
  rewrite H4. apply H3. apply env_one_more_type_bind. rewrite HeqΓ'' in H. auto. auto.
  apply IHtype_bind. auto. auto. lia. lia.
  rewrite free_var_in_type_subst. auto. rewrite free_var_in_type_shift. auto. auto.
Qed.

Lemma type_bind_env: forall Γ t T, Γ |- t : T -> env Γ.
intros. induction H. auto. auto. auto. auto. auto. auto. auto. Qed.

Lemma env_shift_prep: forall Γ' Γ m, env (Γ' ++ Γ)
-> m = count_env_type_bind Γ' -> exists Γ0, env_shift 0 m Γ0 = Γ.
intros. remember (Γ' ++ Γ) as Γ''. generalize dependent Γ'. generalize dependent Γ.
generalize dependent m. induction H.
- intros. destruct Γ'. simpl in H0. subst. exists Γ. rewrite env_shift_zero. auto.
  inversion HeqΓ''.
- intros. destruct Γ'. simpl in H0. subst. exists Γ0. rewrite env_shift_zero. auto.
  simpl in HeqΓ''. inversion HeqΓ''. assert (Γ = env_reverse_shift 0 (Γ' ++ Γ0)).
  symmetry in H3. rewrite H3. rewrite env_shift_inversion. auto.
  rewrite env_reverse_shift_split in H1. apply IHenv with (m:=m-1) in H1.
  destruct H1. exists x. assert (env_shift 0 1 (env_shift 0 (m - 1) x) = Γ0).
  rewrite H1. rewrite env_shift_inversion_reverse. auto. rewrite env_shift_combine in H4.
  assert (1 + (m - 1) = m). symmetry in H2. rewrite H2 in H0. simpl in H0. lia.
  rewrite H5 in H4. auto. rewrite H0. symmetry in H2. rewrite H2. simpl.
  pose proof (count_env_type_bind_reverse). symmetry in H4. rewrite H4. lia.
- intros. destruct Γ'. simpl in H0. rewrite H0. exists Γ0. rewrite env_shift_zero.
  auto. simpl in HeqΓ''. inversion HeqΓ''. apply IHenv with (m:=m) in H3.
  auto. symmetry in H2. rewrite H2 in H0. simpl in H0. auto.
Qed.

Lemma type_shift_ignore_base: forall c m T S, type_shift c m T = S ->
type_shift (c + m) 1 S = type_shift c 1 S.
intros ? ? ?. generalize dependent m. generalize dependent c. induction T.
- intros. simpl in H. subst. simpl. auto.
- intros. simpl in H. remember (c <=? n) as b. symmetry in Heqb. destruct b. 
  subst. simpl. apply Nat.leb_le in Heqb. assert ((c + m <=? n + m) = true).
  apply Nat.leb_le. lia. rewrite H. assert ((c <=? n + m) = true).
  apply Nat.leb_le. lia. rewrite H0. auto. apply Nat.leb_gt in Heqb.
  subst. simpl. assert ((c + m <=? n) = false). apply Nat.leb_gt. lia.
  assert ((c <=? n) = false). apply Nat.leb_gt. lia. rewrite H. rewrite H0. auto.
- intros. simpl in H. inversion H. simpl. rewrite IHT1.
  assert (type_shift (c + m) 1 (type_shift c m T2) = type_shift c 1 (type_shift c m T2)).
  rewrite IHT2. auto. auto. rewrite H1. auto. auto.
- intros. simpl in H. subst. simpl. pose proof (IHT (c+1) m (type_shift (c+1) m T)).
  assert (c + 1 + m = c + m + 1). lia. rewrite H0 in H. rewrite H. auto. auto.
Qed.

Lemma env_shift_ignore_base: forall m Γ Γ', env_shift 0 m Γ = Γ' ->
env_shift m 1 Γ' = env_shift 0 1 Γ'.
intros ? ?. generalize dependent m. induction Γ.
- intros. simpl in H. subst. simpl. auto.
- intros. destruct a. simpl in H. destruct Γ'. inversion H.
  inversion H. simpl. pose proof (type_shift_ignore_base 0 m t (type_shift 0 m t)).
  simpl in H0. rewrite H0. rewrite IHΓ. auto. auto. auto.
  simpl in H. destruct Γ'. inversion H. inversion H. simpl. rewrite IHΓ.
  auto. auto.
Qed.
...
rewrite env_shift_split in H1. simpl in H1. apply type_bind_env in H.
apply env_shift_prep with (m:=m) in H. destruct H. apply env_shift_ignore_base in H.
rewrite H in H1. auto. auto. 
Qed.
...
- intros. destruct Γ'. simpl in HeqΓ''. simpl in H0. simpl in H1.
  ...
  assert (n = count_env_var_bind (env_reverse_shift 0 Γ')). pose proof count_env_var_bind_reverse.
...
- intros. simpl in H. simpl in H0. subst. assert ((0 =? 0) = true). apply Nat.eqb_eq. auto.
rewrite H0 in H. inversion H. auto.
- intros. destruct a. simpl in H. remember (k =? 0) as b. destruct b. simpl in H0. symmetry in Heqb.
  apply Nat.eqb_eq in Heqb. lia. pose proof (IHΓ' _ _ _ _ H). simpl in H0. rewrite H0 in H1.
  apply H1. lia. simpl in H. simpl in H0. apply (IHΓ' _ _ _ _ H). auto.
Qed.

Lemma shift_inversion: forall t c, reverse_shift c (shift c 1 t) = t.
induction t.
- intros. simpl. auto.
- intros. simpl. rewrite IHt. auto.
- intros. simpl. rewrite IHt. auto.
...
- intros. simpl. remember (c <=? n) as b. destruct b. simpl. assert ((c <=? n + 1) = true).
  apply Nat.leb_le. symmetry in Heqb. apply Nat.leb_le in Heqb. lia. rewrite H.
  assert (Init.Nat.pred (n + 1) = n). lia. auto. simpl. symmetry in Heqb. rewrite Heqb.
  auto.
- intros. simpl. rewrite IHt. auto.
- intros. simpl. rewrite IHt. auto.
Qed.

Lemma shift_combine2: forall t n c, shift (c + n) 1 (shift c n t) = shift c (n + 1) t.
induction t.
- intros. simpl. auto.
- intros. simpl. rewrite IHt. auto.
- intros. simpl. assert (S (c + n) = (S c) + n). lia. rewrite H. rewrite IHt. auto.
- intros. simpl. rewrite IHt1. rewrite IHt2. auto.
- intros. simpl. remember (c <=? n) as b. destruct b. simpl. symmetry in Heqb.
  assert ((c + n0 <=? n + n0) = true). apply Nat.leb_le. apply Nat.leb_le in Heqb. lia.
  rewrite H. assert (n + n0 + 1 = n + (n0 + 1)). lia. rewrite H0. auto.
  simpl. symmetry in Heqb. assert ((c + n0 <=? n) = false). apply Nat.leb_gt in Heqb.
  apply Nat.leb_gt. lia. rewrite H. auto.
- intros. simpl. rewrite IHt. auto.
- intros. simpl. rewrite IHt. auto.
Qed.

Lemma get_var_type_larger: forall Γ' n S Γ k T, get_var_type k (Γ' ++ env_var_bind S :: Γ) = Some T 
-> n = count_env_var_bind Γ' -> n < k -> get_var_type (Init.Nat.pred k) (Γ' ++ Γ) = Some T.
induction Γ'.
- intros. simpl in H. remember (k =? 0) as b. destruct b. symmetry in Heqb. 
  apply Nat.eqb_eq in Heqb. lia. simpl. auto.
- intros. destruct a. simpl in H. remember (k =? 0) as b. destruct b. 
  symmetry in Heqb. apply Nat.eqb_eq in Heqb. lia. simpl in H0.
  pose proof (IHΓ' (n-1) S Γ (k-1) T). simpl. assert ((Init.Nat.pred k =? 0) = false).
  apply Nat.eqb_neq. lia. rewrite H3. assert (Init.Nat.pred k = k - 1). lia.
  rewrite H4 in H. rewrite H4. apply H2. auto. lia. lia.
  simpl. simpl in H. simpl in H0. apply IHΓ' with (n:=n)(S:=S). auto. auto. auto.
Qed.

Lemma get_var_type_smaller: forall Γ' n S Γ k T, get_var_type k (Γ' ++ env_var_bind S :: Γ) = Some T 
-> n = count_env_var_bind Γ' -> k < n -> get_var_type k (Γ' ++ Γ) = Some T.
induction Γ'.
- intros. simpl in H. simpl in H0. lia.
- intros. destruct a. simpl in H. remember (k =? 0) as b. destruct b. 
  symmetry in Heqb. apply Nat.eqb_eq in Heqb. rewrite Heqb. simpl. auto.
  simpl. symmetry in Heqb. rewrite Heqb. apply IHΓ' with (n:=n-1)(S:=S). auto. simpl in H0.
  lia. apply Nat.eqb_neq in Heqb. lia. simpl. simpl in H. apply IHΓ' with (n:=n)(S:=S).
  auto. auto. auto.
Qed.
...
- intros. simpl. remember (k =? n) as b. destruct b.
  + assert (reverse_shift n (shift 0 (n + 1) s') = shift 0 n s').
  pose proof (shift_combine2 s' n 0). simpl in H6. symmetry in H6. rewrite H6.
  rewrite shift_inversion. auto. rewrite H6.
  pose proof (env_type_shift_and_shift Γ' s S Γ0 n m). 
  ...
  apply get_var_type_larger with (n:=n) (S:=S'). rewrite HeqΓ'' in H0. auto. auto. auto.
  symmetry in Heqb0. apply Nat.leb_gt in Heqb0. constructor. apply env_one_less_var_bind with (T:=S').
  rewrite HeqΓ'' in H. apply H. apply get_var_type_smaller with (n:=n)(S:=S').
  rewrite HeqΓ'' in H0. apply H0. auto. auto.
- intros. simpl. constructor. apply env_one_less_var_bind with (T:=S'). rewrite HeqΓ'' in H.
